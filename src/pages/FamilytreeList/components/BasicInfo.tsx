...
const BasicInfo: React.FC = () => {
    ...
    const handleImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            if (!validTypes.includes(file.type)) {
                showError('Định dạng file không hợp lệ. Vui lòng chọn file JPG, JPEG, PNG hoặc GIF.');
                return;
            }
            ...
            if (file.size > maxSize) {
                showError('Kích thước file vượt quá 25MB. Vui lòng chọn file nhỏ hơn.');
                return;
            }
    ...
    const handleDelete = async () => {
        if (!selectedTree?.id) {
            showError('Không tìm thấy gia phả để xóa!');
            return;
        }
        ...
        try {
            ...
            showError(error?.response?.data?.Message || 'Có lỗi xảy ra khi xóa gia phả!');
        }
    };
