...
const FamilyTreeSelection: React.FC = () => {
    ...
    const fetchFamilyTrees = async () => {
        try {
            ...
            showError('Không thể tải danh sách gia tộc');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSaveNewTree = async () => {
        ...
        try {
            ...
            showError(err?.response?.data?.message || 'Có lỗi xảy ra khi tạo gia tộc');
        } finally {
            setApiLoading(false);
        }
    };
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                            {availableFamilyTrees.map((tree) => (
                                <div
                                    ...
                                    className={`group bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer border-2 overflow-hidden transform hover:-translate-y-1 ${selectedTreeId === tree.id
